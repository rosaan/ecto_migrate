defmodule Ecto.Migration.Auto.Field do
  ...
  def update_meta(repo, module, tablename, relateds) do
    metainfo =
      ...
      |> Stream.map(&field_to_meta(&1, module, relateds))
      |> Enum.join(",")

    updated_info = %{:tablename => tablename, :metainfo => metainfo}
    field = repo.get!(SystemTable, tablename)

    if field do
      field
      |> SystemTable.changeset(updated_info)
      |> repo.update()
    else
    ...
    case List.keyfind(relateds, field, 0) do
      nil ->
        (field |> Atom.to_string()) <>
          ":" <> (module.__schema__(:type, field) |> type |> Atom.to_string())
  ...
  defp add(module, all_fields, fields_in_db, relateds, {related_key, related_mod})
       when is_list(all_fields) do
      ...
      case List.keyfind(relateds, name, 0) do
        nil ->
          unless name == related_key do
            ...
            add(
              ...
              association_table,
              fields_in_db,
              quote(do: [Ecto.Migration.references(unquote(association_table))])
            )
          end
        ...
        {related_field_name, _mod, association_table} ->
          ...
          add(
            ...
            association_table,
            fields_in_db,
            quote(do: [Ecto.Migration.references(unquote(association_table)), unquote(opts)])
          )
      end
    end
    |> List.flatten()
